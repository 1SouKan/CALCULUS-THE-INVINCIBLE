library(shiny)
source("list1.R")
myData <- read.csv2('Для оценки прогрессирования ФП.csv')
...
shinyServer(function(input, output) {
  ...
  observeEvent(input$confirmChoice, {
    ...
    for(i in input$predChoice){
      insertUI(
        selector = "#here",
        where = "afterEnd",
        #ui = numericInput(i, i, 0)
         # ui = textInput(paste0("txt", input$confirmChoice),
         #                i)
        
         #ui = numericInput(i,i,0)
         #ui=numericInput(inputId = paste0("num2", i),i,0)
         ui=numericInput(i,i,0)
      )
      #inserted <<- c(i, inserted)
    }
    #}
  })
  
  observeEvent(input$resetChoice,{
    for(i in input$predChoice){
    removeUI(
      #selector = paste0('#', inserted[length(inserted)])
      #selector = "div:has(> #here)"
      #selector = paste0("div:has(> #num2", i, ")")
      #selector = "div:has(>paste0('#',i))"
      selector = paste0("div:has(> #", i, ")")
    )
  ...
  observeEvent(input$do, {
    ...
    print(frm)
    glmData <- glm(frm, binomial(link = "logit"), myData)
    print(glmData)
    ...
    print(prMass)
    finData <- predict(glmData, type = "response", prMass)
    print(finData)
    
    # removeUI(
    # selector = "div:has(> #riskOut)"
    # )
    
    output$riskOut<-renderUI({
      textInput("riskOut","", round(finData,2))
    })
    ...
    #   ui = tags$div(
    #     #tags$p(paste(finData)), 
    #     tags$p(paste(round(finData,2))),
    #     id = id
    #   )
    # )
